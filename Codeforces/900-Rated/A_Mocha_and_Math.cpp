...
// Solve function
/*
Because every operation only removes 1-bits; it can never create a bit that wasn't already present in both operands.

Therefore:

A bit that is 1 in every element can survive.
A bit that is 0 in even one element cannot survive in the final common value.
So the smallest achievable maximum is exactly the AND of the entire array.
*/
void solve() {
    int N;cin>>N;
    vector<int> V(N);
